...
def test_health_ok_when_model_and_metadata_present(monkeypatch, tmp_path):
    ...
    metadata_path.write_text(
        json.dumps({"model_version": "prototype_model_d_v1"}), encoding="utf-8"
    )
...
def test_score_200_shape_and_disclaimer(monkeypatch):
    ...
    monkeypatch.setattr(
        bridge_app, "_read_metadata_json", lambda: {"aviso": "AVISO DE PRUEBA"}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        FileNotFoundError(
            2, "No such file or directory", "/app/data/raw/dmc_meteo_x.json"
        ),
        FileNotFoundError(
            2, "No such file or directory", "/app/data/processed/nasa_firms.csv"
        ),
...
def test_score_503_on_real_corrupt_dmc_json(monkeypatch, tmp_path):
    ...
    (tmp_path / "dmc_meteo_2026-09-20.json").write_text(
        '{"330007": {"datos"', encoding="utf-8"
    )
...
def _bridge_block() -> list[str]:
    block = _service_block(
        (REPO_ROOT / "docker-compose.yml").read_text(encoding="utf-8"), "n8n-bridge"
    )
...
def _bridge_config_lines() -> list[str]:
    ...
    dockerfile = (
        (REPO_ROOT / "Dockerfile.n8n-bridge").read_text(encoding="utf-8").splitlines()
    )
    return [
        line.split("#", 1)[0]
...
def test_bridge_does_not_receive_env_file():
    assert not any(
        line.strip().startswith("env_file") for line in _bridge_config_lines()
    )
...
def test_bridge_published_only_on_loopback_8600():
    assert _list_entries(_bridge_block(), "ports") == [
        f"127.0.0.1:{BRIDGE_PORT}:{BRIDGE_PORT}"
    ]
...
def test_bridge_dockerfile_never_copies_env_explicitly():
    ...
    copies = [
        line
        for line in _bridge_config_lines()
